...
from ._knot_search cimport knot_search, MultipleOutcomeDependentData, PredictorDependentData, \
    KnotSearchReadOnlyData, KnotSearchState, KnotSearchWorkingData, KnotSearchData, petha_limit
...
cdef class ForwardPasser:
    ...
    #xx 1. petha_value_list return from next_pair function
    #xx we should print petha at the end of run forward_passer
    #xx petha should be median not mean (to cancel the effect large magnitudes)
    cpdef run(ForwardPasser self):
        ...
        petha_limit_value = median(self.petha_list)
        if petha_limit_value > 3*self.petha or petha_limit_value < 1/3*self.petha:
            warnings.warn('petha value is not within the optimal range')
        petha_recommendation = [(0.333)*petha_limit_value, 3*petha_limit_value]

        print('Optimal petha should be searched in the range of:', petha_recommendation)
        print('self.petha_list',self.petha_list)
...
#                         candidates = np.array(predictor.x)[candidates_idx]
                        ...
                        petha_limit_ = petha_limit(search_data, candidates, p, q, 
                                                   self.m, len(candidates), self.n_outcomes, 
                                                   self.verbose, self.disparity_matrix)
                        petha_limit_value = petha_limit_[0]

                        if not isnan(petha_limit_value) and petha_limit_value != 0:
                           # print('petha_limit_value',petha_limit_value)
                           self.petha_list.append(petha_limit_value)
                           print('self.petha_list',self.petha_list)
