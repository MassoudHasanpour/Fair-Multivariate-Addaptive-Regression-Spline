...
class Earth(BaseEstimator, RegressorMixin, TransformerMixin):
    ...
    def fit(self, X, y=None,
            ...
            output_weight=None,
            missing=None,
            disparity_indices=None, petha=0,
        ...
        self.linear_fit(X, y, sample_weight, output_weight, missing, disparity_matrix,
                        skip_scrub=True)
        return self
    ...
    def linear_fit(self, X, y=None, sample_weight=None, output_weight=None,
                   missing=None, disparity_matrix=None, skip_scrub=False):
        '''
        ...
        for i in range(y.shape[1]):
            ...
            print(sensitive_weights)
            # finding coefficients of each sensitive group
            coef_race = []
            resid_race = []
            for sens in range(disparity_matrix.shape[1]):
                indices = np.where(disparity_matrix[:, sens] == 1)[
                    0]  # a np array with 2 elements. index and type, e.g. finds 1s in each column. row number where each race ==1.  then moves to next column.
                coef_r, resid_r = linalg_mod.lstsq(B[indices, :], weighted_y[indices, i])[
                                  0:2]  # to get the coeffs for each race[indices][0]
                coef_race.append(coef_r)
                resid_race.append(resid_r)

            coef = np.zeros(coef_race[0].shape)
            # resid = np.zeros(resid_race[0].shape)
