...
cdef inline void fast_update_disparity(PredictorDependentData predictor, SingleOutcomeDependentData outcome,
                        ...
                        ,  cnp.ndarray disparity_matrix, int verbose) except *:
...
cpdef tuple petha_limit(KnotSearchData data, FLOAT_t[:] candidates, FLOAT_t[:] p, INDEX_t q, INDEX_t m,
                 INDEX_t r, INDEX_t n_outcomes, int verbose, cnp.ndarray disparity_matrix):
    ...
    cdef list zeta_squared_list = []
    cdef list ASR_dif_list = []
...
#             omega_minus_theta_squared = outcome.omega - np.dot(outcome.theta[:q], outcome.theta[:q])
            ...
            if len(disparity_matrix) != 0:
                for j in range(n_outcomes):
                    ...
                    working.state.ord_idx = 0
                    working.state.idx = predictor.order[working.state.ord_idx]
                fast_update_disparity(predictor, outcome, working, p, q, m, r, disparity_matrix, verbose)
            ...
            #print 'zeta_squared=' , zeta_squared
            #print 'working.ASR_dif_Max=' , working.ASR_dif_Max
        # Compare against best result so far

        #petha_limit
        if working.state.zeta_squared > 0:
            zeta_squared_list.append(working.state.zeta_squared)



        if working.ASR_dif_Max > 0:
            ASR_dif_list.append(working.ASR_dif_Max)

        petha_limit_ = np.mean(zeta_squared_list)/np.mean(ASR_dif_list)

        #print('np.mean(zeta_squared_list)',np.mean(zeta_squared_list))
        #print('np.mean(ASR_dif_list)',np.mean(ASR_dif_list))
